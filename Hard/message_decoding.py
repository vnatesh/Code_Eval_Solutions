...
master=[]
while y<8:
    master+=map(''.join,list(itertools.product('01',repeat=y))[:-1])
    y+=1
...
for test in test_cases.read().split('\n'):
    if test!='':
        final=''
        i=re.split(r'\D+',test)[1]
        i=''.join(re.split(r'\r',i))
        chars=''.join(re.split(r'\d+',test))
        while i!='000':
            a=i[:3]
            b=i[3:]
            ...
            w=0
            while w<len(b):
                list2.append(b[w:w+bin_to_int(a)])
                w+=bin_to_int(a)
            c=''.join(list2[:list2.index('1'*bin_to_int(a))])
            ...
            while x<len(c):
                x+=bin_to_int(a)
                list1.append(c[x-bin_to_int(a):x])
            final+=''.join([chars[p] for p in  map(master.index,list1)])
            i=b[len(c)+bin_to_int(a):]
        print final
